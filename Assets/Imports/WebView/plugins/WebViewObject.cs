...
public class WebViewObject : MonoBehaviour {
    ...
    SimpleWebBrowser.WebBrowser2D webView;
    ...
    public void Init (
        ...
        GameObject canvasGO = new GameObject ("WebViewCanvas");
        canvasGO.transform.parent = transform;
        Canvas canvas = canvasGO.AddComponent<Canvas> ();
        canvas.renderMode = RenderMode.ScreenSpaceOverlay;
        canvas.sortingOrder = 1000;
        canvasGO.AddComponent<GraphicRaycaster> ();
        GameObject webViewPanel = new GameObject ("WebViewPanel");
        webViewPanel.transform.parent = canvasGO.transform;
        webViewPanel.AddComponent<RawImage> ();
        RectTransform rect = webViewPanel.GetComponent<RectTransform> ();
        rect.anchorMin = new Vector2 (0, 0);
        rect.anchorMax = new Vector2 (1, 1);
        rect.pivot = new Vector2 (0, 0);
        webView = webViewPanel.AddComponent<SimpleWebBrowser.WebBrowser2D> ();
        webView.GetComponent<RawImage> ().color = new Color (0, 0, 0, 0);
#elif UNITY_STANDALONE_OSX
        {
...
#elif UNITY_STANDALONE_WIN || UNITY_STANDALONE_LINUX
        Destroy (webView.transform.parent);
    ...
    public void SetMargins (int left, int top, int right, int bottom, bool relative = false) {
        ...
        RectTransform rect = webView.GetComponent<RectTransform> ();
        rect.offsetMin = new Vector2 (ml, mb);
        rect.offsetMax = new Vector2 (mr, mt);
#elif UNITY_WEBPLAYER
...
#elif UNITY_STANDALONE_WIN || UNITY_STANDALONE_LINUX
        webView.GetComponent<RawImage> ().color = Color.white;
#elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
        if (webView == IntPtr.Zero)
            return;
